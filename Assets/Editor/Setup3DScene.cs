...
public class Setup3DScene
{
    ...
    {
        // Guard: don't build while scripts are still compiling
        if (EditorApplication.isCompiling)
        {
            EditorApplication.Beep();
            Debug.LogError("Build cancelled: scripts are still compiling. Wait for compilation to finish, then try again.");
            return;
        }
        ForceCleanCachedScene();
        ...
        // to disk so the scene save below has valid GUID references.
    ...
    {
        // Destroy ALL GameObjects in the current scene for a clean slate
    ...
    static GameObject EnsureBarbarianPrefab()
    {
        ...
        string barbPath = Kayak + "/Characters/fbx/Barbarian.fbx";
        var prefab = AssetDatabase.LoadAssetAtPath<GameObject>(barbPath);
        if (prefab == null)
        {
            Debug.LogWarning($"Barbarian FBX not found at {barbPath}, enemy spawner will skip barbarians.");
            return null;
        }
    ...
    static void PlacePlayer()
    {
        ...
        if (prefab == null) { Debug.LogWarning($"Knight.fbx not found at {basePath}/Knight.fbx! Player not placed."); return; }
        ...
        // The Knight.fbx has no Animator (static mesh), so we add one.
        var heroAnimator = go.GetComponentInChildren<Animator>();
        if (heroAnimator == null)
        {
            heroAnimator = go.AddComponent<Animator>();
            // Load the avatar from the FBX sub-asset (auto-generated by Humanoid import)
            var avatar = AssetDatabase.LoadAssetAtPath<Avatar>(Kayak + "/Characters/fbx/Knight.fbx");
            if (avatar != null) heroAnimator.avatar = avatar;
            Debug.Log($"PlacePlayer: added Animator (avatar: {(heroAnimator.avatar != null ? heroAnimator.avatar.name : "null")})");
        }
        else
            Debug.Log("PlacePlayer: found existing Animator");
